...
1. `Collectors.toMap()` 相同的 key 会抛出 `IllegalStateException` value 为 null 时，会抛出 NullPointException
2. `Arrays.asList()` 转为集合时，生成的集合不支持添加，删除等更新操作，对数组或集合中元素进行修改，会导致对方同步更新。
3. List 的 `subList()` 生成的子列表与原 List 存在关联关系，对原列表进行更新操作，会同步至子列表，对子列表更新也会同步至原列表。
更新操作可能会导致 `ConcurrentModificationException`。

## 序列化

序列化场景：在当前进程之外保存/传输对象，并在需要的时候重新获得对象。

序列化注意事项：
1. 对于非静态的内部类，不应该实现序列化操作。
2. 如果必须实现序列化，实现 `Serializable` 接口的可序列化类，同时显示声明 `SerialVersionUID`，并设置固定值，
保证序列化对象在跨平台，不同版本之间的兼容性。
3. 用 transient 修饰敏感属性，防止敏感信息被序列化。

序列化的不足：
1. 序列化不必要地对外公开了对象的物理实现。
2. 序列化容易使一个类对最初的内部表示产生依赖。（？）
3. 编写正确的反序列化代码有很大的挑战。
4. 序列化增加了安全风险。
5. 序列化增加了测试的难度。
6. 
